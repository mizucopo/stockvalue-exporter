...
class BaseView(MethodView):
    ...
    def __init__(self, app_instance: AppProtocol | None = None) -> None:
        ...
        if app_instance is not None:
            ...
            raise RuntimeError(
                "App instance not set. "
                "Call BaseView.set_app_instance(app) during application setup."
            )
    ...
    def _parse_symbols_parameter(self) -> list[str]:
        ...
        if not config.validate_symbols(final_symbols):
            raise ValueError(
                ...
                f"Max {config.MAX_SYMBOLS_PER_REQUEST} symbols allowed, "
                "each must be 1-15 characters "
                f"(alphanumeric, periods, hyphens). Received: {final_symbols}"
            )
