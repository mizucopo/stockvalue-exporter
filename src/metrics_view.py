...
class MetricsView(BaseView):
    ...
    def _update_change_metrics(
        ...
        # 統一メトリクス: financial_previous_close, financial_price_change,
    ...
    def update_prometheus_metrics(self, stock_data_dict: dict[str, Any]) -> None:
        """各資産タイプのデータでPrometheusメトリクスを更新する.

        Args:
            stock_data_dict: 銘柄別の金融データ辞書
        """
    ...
    def _get_clear_flags(self) -> tuple[bool, bool, bool, bool]:
        ...
        # force_clear_metricsは、リクエスト内容に関係なく全メトリクスを
        # 強制的にクリアする。
    ...
    def _log_request_info(self, symbols: list[str], clear_metrics: bool) -> None:
        ...
        logger.info(
            ...
            f"(url: {url_clear}, auto: {auto_clear}, "
            f"force_clear: {force_clear_metrics})"
        )
