...
class TestMetricsView:
    ...
    def test_update_prometheus_metrics_with_exception(
        ...
        metrics_view = MetricsView(app_instance=mock_app)
        with patch("src.metrics_view.logger") as mock_logger:
    ...
    def test_get_method_with_default_symbols(
        ...
        with patch("src.base_view.request") as mock_request:
            with patch("src.metrics_view.generate_latest") as mock_generate:
                mock_args = MagicMock()
    ...
    def test_get_method_with_clear_parameter(
        ...
        with app.test_request_context("/?symbols=AAPL&clear=true"):
            with patch("src.config.config.AUTO_CLEAR_METRICS", False):
                # 独立したレジストリでMetricsFactoryを作成
                from src.metrics_factory import MetricsFactory
    ...
    def test_get_method_with_auto_clear_config(
        ...
        with app.test_request_context("/?symbols=AAPL"):
            with patch("src.config.config.AUTO_CLEAR_METRICS", True):  # 自動クリア有効
                # 独立したレジストリでMetricsFactoryを作成
                from src.metrics_factory import MetricsFactory
    ...
    def test_get_method_without_clear(
        ...
        with app.test_request_context("/?symbols=AAPL"):
            with patch("src.config.config.AUTO_CLEAR_METRICS", False):  # 自動クリア無効
    ...
    def test_get_clear_flags_method(self, app: Flask) -> None:
        ...
        with app.test_request_context("/"):
            with patch("src.config.config.AUTO_CLEAR_METRICS", True):
                url_clear, force_clear_metrics, auto_clear, clear_metrics = (
                    view._get_clear_flags()
                )
